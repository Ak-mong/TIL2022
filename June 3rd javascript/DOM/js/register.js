function newRegister(){
    ...
    nameList.insertBefore(newP, nameList.childNodes[0]); //최근이름이 위로 오도록 변경(윗라인 대신)
    userName.value = "";
    ...
    for (var i=0; i < removeBttns.length; i++){
        removeBttns[i].addEventListener("click", function(){
            if(this.parentNode.parentNode){//현재 노드(this)의 부모노드(span)의 부모노드(p)가 있을 경우 실행
                this.parentNode.parentNode.removeChild(this.parentNode);
                //현재노드(this)의 부모 노드의 부모 노드를 찾아서 '현재(this)노드의 부모노드(p)를 삭제
            }
        });
    }
}